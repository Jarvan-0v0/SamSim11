...
<%
		try{
			...
			if(data0.length != 0){
				udao.table_delete("cargo_inspection_log");
				for(int i=0;i<data0.length;i++){
					String[] data = data0[i].split("_");
					u.setData(data);
					udao.cargo_insert(u,15);
				}
			}
			if(data00.length != 0){
				udao.table_delete("cargo_inspection_book");
				for(int i=0;i<data00.length;i++){
					String[] data = data00[i].split("_");
					u.setData(data);
					udao.cargo_insert(u,13);
				}
			}
			...
			%><script type="text/javascript">window.location.href = "cargoLog.html";</script><%
		}
%>
</body>
</html>