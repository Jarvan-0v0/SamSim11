	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$("#status_info").html("共选取 " + num + " 个文件，大小共 "+ size +"。");
			};
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					...
					var ext = (file.name).substring(pos+1);
					//返回-1表示没找到，返回其他值表示找到的索引
					var len = in_array(para.fileType, ext);
					if (len == "-1"){
					    //$("#status_info").html("不支持上传此格式的文件!");
						tipDialog('不支持此格式的文件!', 3, 'warning');
					}
					else if (file.size >= 51200000) {
						//tipDialog(file.name +'"文件太大', 4, 'warning');
						$("#status_info").html(file.name + "文件太大!");
					}
					else {// 在这里需要判断当前所有文件中
							arrFiles.push(file);	
					}	
				}
				return arrFiles;//返回满足条件的文件
			};
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				//stmp.substring(3);//从第4个字符开始,截取到最后个字符;
				// 处理配置参数删除按钮
				var delHtml = "";
				if(para.del){  // 显示删除按钮
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "../scripts/uploadfile/images/fileType/";
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="uploadSuccess_'+file.index+'" class="file_success">成功上传！</p>';
					html += '</div>';
				}
				else{
					html += '<div id="uploadList_'+ file.index +'" class="upload_append_list">';
			...
			this.createCorePlug = function(){
				var params = {
					...
					filterFile: function(files) {//过滤合格的文件
					...
					onSuccess: function(file, response) {
						if(response.indexOf(":") > 0){
							$("#status_info").html(response.substring(6,response.length));
							return;
						}
						else{
							$("#status_info").html("文件成功上传！"+ response);
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						...
						//tipDialog('请先选取拟上传文件!', 3, 'warning');
						$("#status_info").html('请先选取拟上传文件!');
						//alert("请先选中文件再点击上传");
					}
	            });
