...
public class EhcacheUtils 
{
	...
	public static int MAXELEMENTSINMEMORY = 50000; //元素最大数量
	public static boolean OVERFLOWTODISK = true;//是否把溢出数据持久化到硬盘
	public static boolean ETERNAL = false; //是否会死亡
	public static int TIMETOIDLESECONDS = 600;//缓存的间歇时间
	public static int TIMETOlIVESECONDS = 86400;//存活时间(默认一天)
	public static boolean DISKPERSISTENT = false;//需要持久化到硬盘否
	public static String MEMORYSTOREEVICTIONPOLICY = "LFU";//内存存取策略
	...
	public static CacheManager initCacheManager() {
		try {
			...
			Console.showMessage(EhcacheUtils.class.getSimpleName(), e.getMessage());
		}
		return cacheManager;
	}

	/**
	 * 使用指定配置文件创建 CacheManager
	 * 初始化缓存管理容器
	...
	public static CacheManager initCacheManager(String path) {
		try {
			if (cacheManager == null) {
				//cacheManager = CacheManager.getInstance().create(path); XBM
	...
	public static Cache initCache(String cacheName, int maxElementsInMemory, boolean overflowToDisk, boolean eternal,
		...
		try {
			...
			if (myCache != null) {
				...
				config.setMaxEntriesInCache(maxElementsInMemory);//.setMaxElementsInMemory(maxElementsInMemory);
				config.setOverflowToOffHeap(overflowToDisk);//.setOverflowToDisk(overflowToDisk);
	...
	public static Object getValue(String cacheName, String key) throws Exception {
		try {
			...
			return myCache.get(key).getObjectValue();//.getValue();
		} catch (Exception e) {
			return null;
		}
	}
	...
	public static boolean removeAllEhcache(String cacheName) throws Exception {
		try {
			CacheManager myManager = CacheManager.create();
			myManager.removeAllCaches();//.removalAll();
			return true;
		} catch (Exception e) {
			throw new Exception("remove cache " + cacheName + " failed!!!");
		}
	...
	public static void removeAllCache() {
		checkCacheManager();
		cacheManager.removeAllCaches();//xbm .removalAll();
	}
	...
	public static void main(String[] arg) {
		...
		for (int i = 0; i < keys.size(); i++) {
			Console.showMessage(keys.get(i));
		}
